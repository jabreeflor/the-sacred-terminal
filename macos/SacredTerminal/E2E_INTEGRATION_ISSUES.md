...
- Follow-up update: Session rows are now native row-level `NSButton` controls hosted in a manual-layout `RailTreeView` instead of `NSStackView`; the E2E UI driver verifies both seeded session rows route center-point hits to themselves before pressing them.
...
- Follow-up update: Added E2E-only socket UI driver commands (`ui-state`, `ui-hit-test`, `ui-press`, and `ui-smoke-session-tabs`) behind `SACRED_TERMINAL_E2E=1`. The always-on SwiftPM integration test now launches the real app with isolated fixture state and uses the driver to switch rail sessions both directions, create a terminal tab, and switch terminal tabs back and forth without macOS Accessibility permission.
...
Fix-phase update (session/tab switching follow-up): added a non-Accessibility E2E UI driver over the existing isolated socket and a SwiftPM test for the concrete regression path. The driver verifies rail session row hit routing, switches `s10 -> s12 -> s10`, presses `workspace-new-tab`, then switches `workspace-tab-s11 -> workspace-tab-s100` and asserts the isolated fixture persisted the expected active session/pane state. This does not cover hover-only quick-pick/session-close flows or full process survival across app quit/relaunch.
